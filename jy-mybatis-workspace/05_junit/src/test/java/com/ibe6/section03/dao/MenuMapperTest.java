...
class MenuMapperTest {
    ...
    @DisplayName("존재하지 않는 카테고리 번호로 메뉴 조회시 빈 리스트 반환됨")
    @Test
    void selectMenuByCategoryCode2() {
        int categoryCode = 35351;
        List<MenuDto> list = menuMapper.selectMenuByCategoryCode(categoryCode);
        assertThat(list)
                .isNotNull()
                .isNotEmpty();
    ...
    void insertMenu() {
        ...
        // when
        int result = menuMapper.insertMenu(menu); // insert후에 menu의 menuCode필드에는 현재 생성된 메뉴 번호가 담겨있을 것
        if(result > 0)
            sqlSession.commit();
